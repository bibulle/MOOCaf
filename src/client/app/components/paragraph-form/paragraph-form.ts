...
// Initialisation
  ...
  private _markdownToHtml() {
    ...
    this.data.content['label_html'] = ParagraphAbstract.markdownToHTML(this.data.content['label']).replace(/^<p>(.*)<\/p>[\r\n]*$/, "$1");
    ...
    if (this.data.content['questions']) {
      ...
      this.data.content['questions_html'] = questions.map(s => {
        return ParagraphAbstract.markdownToHTML(s).replace(/^<p>(.*)<\/p>[\r\n]*$/, "$1")
      })
    }
