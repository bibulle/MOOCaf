...
@Component({
  selector: 'paragraph-java',
  templateUrl: './paragraph-java.component.html',
  styleUrls: ['../paragraph.component.css', './paragraph-java.component.css']
})
export class ParagraphJavaComponent extends ParagraphAbstractComponent implements OnInit, AfterViewInit {
  ...
  scrollReturnToBottom(forced: boolean) {
    ...
    const pre = this._el.nativeElement.querySelector('pre');
    if (pre) {
      if (forced || (pre.scrollTop+pre.clientHeight >= pre.scrollHeight)) {
        setTimeout(() => {
